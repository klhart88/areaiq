...
const CATEGORIES = {
  restaurants: {
    label: 'Restaurants',
    emoji: '🍽️',
    radiusMeters: 2400,  // ~1.5 mile radius
  ...
  groceries: {
    label: 'Groceries',
    emoji: '🛒',
    radiusMeters: 3200,  // 2 mile radius for groceries
    query: 'shop~"^(supermarket|grocery|convenience)$"'
  },
  parks: {
    label: 'Parks',
    emoji: '🌳',
...
export async function fetchVenues(location, categoryKey) {
  ...
  const overpassQuery = `
    [out:json][timeout:25];
    (
      node[${category.query}][name](around:${category.radiusMeters},${location.lat},${location.lng});
      way[${category.query}][name](around:${category.radiusMeters},${location.lat},${location.lng});
    );
    out body center 30;
  `;
