...
initAddressAutocomplete(addressInput, { menu: addressSuggestions });
...
async function handleSearch() {
  ...
  searchButton.textContent = 'Building report…';

  showLoading('Building your AreaIQ report…');
  scrollToResults();
...
function showLoading(message) {
  resultsSection.innerHTML = `
    <div class="report-status">
      <div class="report-status-dot" aria-hidden="true"></div>
      <div>
        <div class="report-status-title">${escapeHtml(message)}</div>
        <div class="report-status-copy">We are checking the address first. Your area details will begin appearing below as each section loads.</div>
      </div>
    </div>
  `;
}
...
function showLocationResults(location) {
  resultsSection.innerHTML = `
    <div class="report-status">
      <div class="report-status-dot" aria-hidden="true"></div>
      <div>
        <div class="report-status-title">Address found. Building your AreaIQ snapshot.</div>
        <div class="report-status-copy">Schools, taxes, commute tools, nearby places, and development trends will appear below as they load.</div>
      </div>
    </div>
...
function appendReportReadyMessage() {
  const block = document.createElement('div');
  block.className = 'message message-success';
  block.textContent = 'Your AreaIQ snapshot is ready. Review the sections above, then save your research or ask Kelvin a question below.';
  resultsSection.appendChild(block);
}

function scrollToResults() {
  if (!resultsSection) return;
  const offset = window.matchMedia('(max-width: 640px)').matches ? 92 : 118;
  const targetTop = resultsSection.getBoundingClientRect().top + window.pageYOffset - offset;
...
function appendCommuteForm() {
  ...
  block.innerHTML = `
    ...
    <div class="commute-form">
      <div class="commute-address-field address-field">
        <input
          type="text"
          id="commute-destination"
          class="commute-input"
          placeholder="Start typing a work address or destination"
          autocomplete="off"
        >
        <div id="commute-suggestions" class="address-suggestions" hidden></div>
      </div>
      <button id="commute-button" type="button" class="commute-button">Calculate</button>
    </div>
    <p class="address-helper commute-address-helper">Choose a suggested match for the most accurate commute estimate.</p>
  ...
  const commuteSuggestions = document.getElementById('commute-suggestions');

  initAddressAutocomplete(commuteInput, { menu: commuteSuggestions });
...
function getVenueIconHTML(categoryKey) {
  const icons = {
    restaurants: '<svg class="venue-tab-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M7 4v7"></path><path d="M10 4v7"></path><path d="M8.5 11v9"></path><path d="M17 4v16"></path><path d="M14 4c0 4 1 6 3 7"></path></svg>',
    coffee: '<svg class="venue-tab-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M6 8h10v5a4 4 0 0 1-4 4H10a4 4 0 0 1-4-4V8z"></path><path d="M16 10h2a2 2 0 0 1 0 4h-2"></path><path d="M5 20h13"></path></svg>',
    groceries: '<svg class="venue-tab-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M6 6h15l-2 8H8L6 3H3"></path><circle cx="9" cy="19" r="1.5"></circle><circle cx="18" cy="19" r="1.5"></circle></svg>',
    parks: '<svg class="venue-tab-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 4l4 6h-3l4 6h-4v4h-2v-4H7l4-6H8l4-6z"></path></svg>',
    fitness: '<svg class="venue-tab-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M5 9v6"></path><path d="M19 9v6"></path><path d="M8 8v8"></path><path d="M16 8v8"></path><path d="M8 12h8"></path></svg>',
    pharmacies: '<svg class="venue-tab-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 5v14"></path><path d="M5 12h14"></path><path d="M7 7h10v10H7z"></path></svg>',
    gas: '<svg class="venue-tab-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M7 4h8v16H7z"></path><path d="M9 7h4"></path><path d="M15 8h2l2 3v6a2 2 0 0 1-2 2"></path></svg>'
  };
  return icons[categoryKey] || '<svg class="venue-tab-icon" viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="6"></circle></svg>';
}
...
function appendVenuesSection() {
  ...
  const tabButtons = categories.map((cat, idx) => `
    <button
      ...
      ${getVenueIconHTML(cat.key)}<span>${cat.label}</span>
    </button>
  `).join('');
    ...
    resultsDiv.innerHTML = venues.map(v => `
      <div class="venue-card">
        ...
        <div class="venue-meta">
          ...
          ${v.phone ? ` · <a href="tel:${escapeHtml(window.phoneToTel ? window.phoneToTel(v.phone) : toTelHref(v.phone))}">${escapeHtml(window.formatPhoneDisplay ? window.formatPhoneDisplay(v.phone) : formatPhoneDisplay(v.phone))}</a>` : ''}
        </div>
        <a class="venue-link"
...
function formatCountyLabel(countyName) {
  if (!countyName) return 'the county';
  return /county$/i.test(String(countyName).trim())
    ? String(countyName).trim()
    : `${String(countyName).trim()} County`;
}
...
function appendDevelopmentResults(dev) {
  ...
  block.innerHTML = `
    ...
    <p class="feature-description">
      Housing growth in <strong>${escapeHtml(formatCountyLabel(dev.countyName))}</strong> over the last ${dev.yearsSpanned} years.
    </p>
...
// ---------- Phone helpers ----------
// Kept inside app.js so Nearby Places can render phone links without relying
// on helpers from another module or an older cached file.
...
if (backToTopButton) {
  let backToTopTimer;
  let backToTopIsHeld = false;

  const hideBackToTop = () => {
    if (!backToTopIsHeld) backToTopButton.classList.remove('is-visible');
  };

  const scheduleBackToTopHide = () => {
    window.clearTimeout(backToTopTimer);
    backToTopTimer = window.setTimeout(hideBackToTop, 1400);
  };

  const showBackToTopWhileScrolling = () => {
    window.clearTimeout(backToTopTimer);
  ...
  backToTopButton.addEventListener('pointerenter', () => {
    backToTopIsHeld = true;
    window.clearTimeout(backToTopTimer);
    backToTopButton.classList.add('is-visible');
  });
  backToTopButton.addEventListener('pointerleave', () => {
    backToTopIsHeld = false;
    scheduleBackToTopHide();
  });
  ...
  backToTopButton.addEventListener('blur', () => {
    backToTopIsHeld = false;
    scheduleBackToTopHide();
  });

  window.addEventListener('scroll', showBackToTopWhileScrolling, { passive: true });
  backToTopButton.addEventListener('click', (event) => {
    event.preventDefault();
    window.scrollTo({ top: 0, behavior: 'smooth' });
    window.setTimeout(() => backToTopButton.classList.remove('is-visible'), 900);
  });
  hideBackToTop();
}

// ---------- Email link auto-search ----------
// Reads ?address= from the URL (set by the email CTA link)
// and automatically runs the search so results load on arrival.
