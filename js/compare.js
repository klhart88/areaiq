...
const suggestionMenus = [
  document.getElementById('compare-suggestions-1'),
  document.getElementById('compare-suggestions-2'),
  document.getElementById('compare-suggestions-3')
];
...
function init() {
  ...
  addressInputs.forEach((input, index) => {
    initAddressAutocomplete(input, { menu: suggestionMenus[index] });
  });
  ...
  addressInputs.forEach(input => {
    input.addEventListener('input', updateThirdRowState);
...
function hideThirdRow() {
  ...
  if (suggestionMenus[2]) suggestionMenus[2].hidden = true;
  updateThirdRowState();
}

function updateThirdRowState() {
  const thirdHasValue = Boolean(addressInputs[2]?.value.trim());

  // If a shared link or browser restore provides a third address, reveal the row.
  if (thirdHasValue) {
    row3.hidden = false;
  }

  const thirdRowVisible = !row3.hidden;
...
async function handleCompare() {
  ...
  for (let i = 0; i < addressInputs.length; i++) {
    ...
    const rowVisible = i < 2 || !row3.hidden;
  ...
  try {
    ...
    // Move the user to the top of the results card once results are ready.
...
function scrollToCompareResults() {
  ...
  const scrollToCard = () => {
    ...
    const breathingRoom = isMobile ? 18 : 24;
    const minimumOffset = isMobile ? 88 : 112;
    const offset = Math.max(minimumOffset, headerHeight + breathingRoom);
  ...
  // Let the results card finish drawing first, especially on mobile where
  // the layout height can change after the comparison table renders.
  requestAnimationFrame(() => {
    window.setTimeout(scrollToCard, isMobile ? 140 : 60);
  });
}
